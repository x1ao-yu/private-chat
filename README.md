# Private Chat — P6 Anonymous Identity

Lightweight browser-based E2EE private chat. P6 adds a session identity: a fresh Ed25519 keypair per room (memory only, cross-room unlinkable), signed chat messages and nickname claims, receive-side signature verification with per-room TOFU pinning, key-change warnings and explicit "no identity" marking for unsigned messages. P5 security (replay protection, integrity, key rotation, cooperative removal, rate limits, XSS/CSP), P4 privacy, P3 room keys, P2 E2EE, P1 minimal chat and P0 foundation already done.
...
frontend/src/
  ...
  identity.ts       # P6 session identity: Ed25519 keypair, fingerprints, signed message/nick wrap+verify
...
`message.from` is an ephemeral connection ID (`peer-` + 4B random per WebSocket, regenerated on reconnect, `backend/internal/ws/transport.go:210`) — **not** a stable identity. Since P6, sender attribution comes from the Ed25519 session identity inside the encrypted payload. No history is persisted (relay-only, `backend/internal/channel/manager.go:11`); rejoin does not replay. Minimal `rate_limited` (`create 5/min per-IP+per-conn`, `send 10/s per-conn`).
...
**P6 identity rides inside the encrypted payload** (zero server/protocol changes): the inner plaintext of chat messages is now JSON `{t:"msg",v:1,text,nick,pk,sig}` and nickname updates `{t:"nick",v:1,nick,pk,sig}`. Messages sign `identity-v1|channelId|messageIdB64|nick|text`; nick claims sign `identity-v1|nick|channelId|nick`. Receivers verify per message (TOFU pinning per room, tab memory), hide content on invalid signatures, and warn on key changes (same nick, different fingerprint — once per contested pair as a system message, plus a connection-level mismatch error). Structured inner payloads from unknown client versions are ignored, never rendered as raw JSON. Unsigned messages are visibly marked with a "no identity" chip; if Ed25519 is unsupported the room header shows "identity auth unavailable" (no silent degradation). The plaintext budget is capped at `MAX_INNER_UTF8 = 6098` bytes before encryption (`codec.ts`); the wire limit stays 8192. See `frontend/src/identity.ts:1` and SECURITY.md "P6 Notes" for the exact boundaries (TOFU only, pseudonymity not anonymity, session-scoped by design).
...
- **Replay/Integrity (P5)**: per-channel `ReplayCache` LRU 1000 on `messageIdB64` (`frontend/src/e2ee.ts:51`, `frontend/src/channel.svelte.ts:106`) drops duplicates, `isValidEnvelope` pre-validates 12B iv/16B mid/≥16B ct; GCM tag + AAD ensures outsider integrity, `MessageList.svelte` text interpolation safe (no `{@html}`), `XSS` payload stays plaintext.
- **Session identity (P6)**: fresh Ed25519 keypair **per room** (memory only, never persisted, cross-room unlinkable); chat messages and nick updates carry `{pk,sig}` inside the E2EE payload, verified per message against per-room TOFU pinning (`frontend/src/identity.ts:1`, `frontend/src/channel.svelte.ts`); invalid signatures hide content (`⚠️ invalid signature`), key changes warn (same nick/different fingerprint → amber chip + one-time system message; two identities on one connection → error), unsigned messages show a gray "no identity" chip, and an unsupported browser shows an explicit "identity auth unavailable" badge. Avatar colors use the stable identity fingerprint. Boundaries: TOFU only, no PKI/out-of-band verification; pseudonymity not anonymity (refresh/leave produces a new room-session identity); unsigned legacy claims remain spoofable — see SECURITY.md "P6 Notes".
