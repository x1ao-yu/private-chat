...
* `isCryptoAvailable()` gate requires secure context (HTTPS or `localhost`); tests run under `jsdom` (currently 30.x), which forwards to Node Web Crypto. That forwarding is load-bearing, so `identity.test.ts` asserts `isIdentityAvailable()` is `true` in the test environment — a jsdom upgrade that drops it fails loudly instead of silently degrading the crypto suites. Tamper of `iv`/`messageId`/`ct`/`AAD` causes `OperationError` decrypt failure, surfaced as `⚠️ decrypt failed`.
